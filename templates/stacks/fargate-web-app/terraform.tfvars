use_init_placeholder_image = true # don't switch to false until the ECR repo is populated (ie: github has pushed an image)

app_name    = "CHANGEME"
app_env     = ""         # if you want to change the env from the terraform stack env (uat/qa/test/stag/etc will all be in terraform 'dev')
github_repo = "CHANGEME" # used for CI/CD deploy role
...
host_headers                = ["CHANGEME.aaa.tools"] # must be unique for that env/loadbalancer. LB must also have https cert that covers the host (manually installed)
alb_https_listener_priority = 500                    #must be unique for that loadbalancer
...
container_image  = "ACCOUNT_ID_CHANGEME.dkr.ecr.ap-southeast-2.amazonaws.com/CHANGEME" # should be the full docker image URI, minus the label - should match the ecr repo indicated above
