...
variable "efs_mounted_volume_paths" {
  # see notes in main.tf/aws_efs_mount_target in the fargate web app module (also README)
  # no more or less than one path, but kept as a list instead of a string in case we fix it later (it changes how the resources are referenced)
  description = "Enable EFS by putting a list of ONE mount path. DO NOT PUT MORE THAN ONE PATH! DO NOT PUT LESS THAN ONE PATH! It will not work"
  type        = list(string)
  default     = ["/tmp/CHANGEME"]
}
...
variable "efs_extra_writemount_security_groups" {
  description = "additional external security groups (eg: backup, efs hax mounter) for nfs"
  type = list(string)
  default = []
}