...
github_repo = "client7" # used for CI/CD deploy role
...
host_headers                = ["dev-client7.aaa.tools"] # must be unique for that env/loadbalancer. LB must also have https cert that covers the host (manually installed)
alb_https_listener_priority = 412                    #must be unique for that loadbalancer
...
container_image  = "123456789012.dkr.ecr.ap-southeast-2.amazonaws.com/client7" # should be the full docker image URI, minus the label - should match the ecr repo indicated above
