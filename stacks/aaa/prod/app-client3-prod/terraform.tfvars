...
github_repo = "client3" # used for CI/CD deploy role
...
host_headers                = ["prod-client3.aaa.tools"] # must be unique for that env/loadbalancer. LB must also have https cert that covers the host (manually installed)
alb_https_listener_priority = 627                    #must be unique for that loadbalancer
...
container_image  = "2345678901234.dkr.ecr.ap-southeast-2.amazonaws.com/client3" # should be the full docker image URI, minus the label - should match the ecr repo indicated above
