...
github_repo = "membership-apple-wallet" # used for CI/CD deploy role
...
host_headers                = ["prod-client6.aaa.tools", "client6.aaa.com.au"] # must be unique for that env/loadbalancer. LB must also have https cert that covers the host (manually installed)
alb_https_listener_priority = 507                    #must be unique for that loadbalancer
...
container_image  = "2345678901234.dkr.ecr.ap-southeast-2.amazonaws.com/client6" # should be the full docker image URI, minus the label - should match the ecr repo indicated above
