...
resource "aws_instance" "logproxy" {
  ami                     = data.aws_ami.debian.id
  instance_type           = var.instance_type
  disable_api_termination = var.disable_api_termination
  iam_instance_profile    = aws_iam_instance_profile.logproxy.id
  vpc_security_group_ids  = [aws_security_group.logproxy.id]
...
resource "aws_iam_instance_profile" "logproxy" {
  name = "ec2-logproxy-profile"
  role = aws_iam_role.logproxy.name
}
resource "aws_iam_role" "logproxy" {
  name = "ec2-logproxy"
...
resource "aws_security_group" "logproxy" {
  #name_prefix has a picosecond suffix... really guys? 26 chars from YYYY on down...
  name_prefix = "ec2-logproxy"
  description = "logproxy sg"
  ...
  ingress = [
    {
      ...
      to_port          = 0
      protocol         = "-1"
      cidr_blocks      = ["172.30.0.0/16", "192.190.180.0/24"] # TODO: parametise this
      ipv6_cidr_blocks = []
      prefix_list_ids  = []
      self             = true
