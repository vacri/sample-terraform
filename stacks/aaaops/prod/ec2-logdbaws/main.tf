...
resource "aws_instance" "logdb" {
  ami                     = data.aws_ami.debian.id
  instance_type           = var.instance_type
  disable_api_termination = var.disable_api_termination
  iam_instance_profile    = aws_iam_instance_profile.logdb.id
  vpc_security_group_ids  = [aws_security_group.logdb.id]
...
resource "aws_iam_instance_profile" "logdb" {
  name = "ec2-logdb-profile"
  role = aws_iam_role.logdb.name
}
resource "aws_iam_role" "logdb" {
  name = "ec2-logdb"
...
resource "aws_security_group" "logdb" {
  #name_prefix has a picosecond suffix... really guys? 26 chars from YYYY on down...
  name_prefix = "ec2-logdb"
  description = "logdb sg"
  vpc_id      = data.terraform_remote_state.vpc.outputs.vpc_id

  ingress = [
    {
      from_port        = 0
      to_port          = 0  # TODO maybe lock this down to just ssh + elasticsearch ports?
      protocol         = "-1"
